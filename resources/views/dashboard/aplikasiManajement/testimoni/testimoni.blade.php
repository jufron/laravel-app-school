<x-partials.layout title="testimoni" pageState="Testimoni">
  <element
    id="data_testimoni"
    x-data="{
      modalCreateTesimoniIsOpen : false,
      modalDeleteTestimoniIsOpen : false,
      modalGetTestimoniIsOpen : false,
      modalToasStatus : false,
      images: '',
      route: '',
     }"
    @error('image')
     x-init="modalCreateTesimoniIsOpen = true"
    @enderror
    @error('name')
     x-init="modalCreateTesimoniIsOpen = true"
    @enderror
      ...
      <div class="flex flex-col gap-10">
        ...
        <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          ...
          <x-small.button.button
            ...
            click="modalCreateTesimoniIsOpen"
            title="Tambah"
          />
                ...
                <tr>
                  ...
                  <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                    <div class="h-15 w-20 rounded-md overflow-hidden">
                      @if ($testimo->image)
                      <img src="{{ asset("storage/$testimo->image") }}" alt="banner" loading="lazy">
                      @else
                  ...
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p class="text-black dark:text-white">{{ $testimo->name }}</p>
                  </td>
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p class="text-black dark:text-white">{{ $testimo->message_limit }}</p>
                  </td>
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p class="text-black dark:text-white">{{ $testimo->created_at }}</p>
                  </td>
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p class="text-black dark:text-white">{{ $testimo->updated_at }}</p>
                  </td>
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <div
                      class="flex items-center space-x-3.5">
                      <button @click="
                        modalGetTestimoniIsOpen = true;
                        route = `{{ route('dashboard.testimoni.get', $testimo) }}`;
                        "
                        id="button_info"
                        data-url="{{ route('dashboard.testimoni.get', $testimo) }}"
                        class="hover:text-primary dark:bg-slate-800 bg-slate-100 rounded-lg w-10 h-10">
                        <i class="fa-solid fa-circle-info"></i>
                      </button>
                      <a
                        href="{{ route('dashboard.testimoni.edit', $testimo) }}"
                      ...
                      <button @click="modalDeleteTestimoniIsOpen = true; route = `{{ route('dashboard.testimoni.destroy', $testimo) }}` " class="hover:text-primary dark:bg-slate-800 bg-slate-100 rounded-lg w-10 h-10">
    ...
    <x-small.modal.modal state="modalCreateTesimoniIsOpen" size="sm:max-w-2xl" >
      <x-slot:modalContent>
        <form action="{{ route('dashboard.testimoni.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="bg-white dark:bg-slate-900 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-3 sm:mt-0 sm:text-left">
                  <h3 class="text-lg font-semibold leading-6 text-gray-900" id="modal-title">Tambah Testimoni</h3>
                  <div class="mt-6">
                    ...
                    <div class="flex w-full border-l-6 border-sky-500 bg-sky-500 bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
                      ...
                      <div class="w-full">
                        ...
                        <p class="leading-relaxed text-sky-400 tet-md">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              ...
              <button
                @click="modalCreateTesimoniIsOpen = false;" type="button"
    ...
    {{-- ? modal info testimoni --}}
    <x-small.modal.modal state="modalGetTestimoniIsOpen">
      <x-slot:modalContent>
        <element @keydown.escape.window="route = '' ">

          <div class="bg-white dark:bg-slate-900 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              <div class="mt-3 sm:mt-0 sm:text-left">
                <h3 class="text-lg font-semibold leading-6 text-gray-900" id="modal-title">Info Testimoni</h3>
                <div class="mt-6" id="modal-info-container">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            <button
              @click="modalGetTestimoniIsOpen = false; route = '' " type="button"
              class="mt-3 inline-flex w-full justify-center rounded-md bg-white dark:bg-slate-700 px-6 py-3 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">
              Close
            </button>
          </div>
    ...
    <x-small.modal.modal state="modalDeleteTestimoniIsOpen" >
      <x-slot:modalContent>
        <form method="post" x-bind:action="route" @keydown.escape.window="route = '' ">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            ...
            <button @click="modalDeleteTestimoniIsOpen = false; route = '' " type="button" class="mt-3 inline-flex w-full justify-center rounded-md bg-white dark:bg-slate-700 px-6 py-3 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button>
          </div>
        </form>
      </x-slot>
    </x-small.modal.modal>
