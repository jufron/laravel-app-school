<x-partials.layout title="galeri" pageState="Galeri">
  <element
    x-data="{
      modalCreateGaleriIsOpen : false,
      modalDeleteGaleryIsOpen : false,
      modalToasStatus: false,
      images: '',
      route: '',
     }"
    @error('image')
     x-init="modalCreateGaleriIsOpen = true"
    @enderror
    @error('description')
     x-init="modalCreateGaleriIsOpen = true"
    @enderror
    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      ...
      <div class="flex flex-col gap-10">
        ...
        <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                    <div class="h-10 w-15 rounded-md overflow-hidden">
                      <img src="{{ asset("storage/$gal->image") }}" alt="banner" loading="lazy">
                    </div>
                  </td>
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p class="text-black dark:text-white">{{ $gal->created_at }}</p>
                  </td>
                  <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <div class="flex items-center space-x-3.5">
                      <a
                        href="{{ route('dashboard.galeri.download', $gal) }}"
                        target="_blank"
                      ...
                      <button @click="modalDeleteGaleryIsOpen = true; route = `{{ route('dashboard.galeri.destroy', $gal) }}` " class="hover:text-primary dark:bg-slate-800 bg-slate-100 rounded-lg w-10 h-10">
          ...
          @if (!$galery->onFirstPage()	)
          <x-small.button.button-link
            className="inline-flex items-center justify-center rounded-lg bg-blue-600 hover:bg-blue-700 px-8 py-3 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 my-6"
            urlHref="{{ $galery->previousPageUrl() }}"
            label="Sebelumnya"
          />
          @endif
          {{-- ."&page=".$galery->count() --}}
          @if ($galery->hasMorePages()	)
          <x-small.button.button-link
            className="inline-flex items-center justify-center rounded-lg bg-blue-600 hover:bg-blue-700 px-8 py-3 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10 my-6"
            urlHref="{{ $galery->nextPageUrl() }}"
            label="Selanjutnya"
          />
          @endif
        </div>
    ...
    {{-- ? modal create banner --}}
    <x-small.modal.modal state="modalCreateGaleriIsOpen" >
      <x-slot:modalContent>
        <form action="{{ route('dashboard.galeri.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bg-white dark:bg-slate-900 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-3 sm:mt-0 sm:text-left">
                  <h3 class="text-lg font-semibold leading-6 text-gray-900" id="modal-title">Tambah Galeri</h3>
                  <div class="mt-6">
                    ...
                    <x-small.input.input-textarea
                      placeholder="Masukan data deskripsi / keterangan"
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              ...
              <button
                @click="modalCreateGaleriIsOpen = false;" type="button"
    ...
    {{-- ? modal delete banner --}}
    <x-small.modal.modal state="modalDeleteGaleryIsOpen" >
      <x-slot:modalContent>
        <form method="post" x-bind:action="route" @keydown.escape.window="route = '' ">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            ...
            <button @click="modalDeleteGaleryIsOpen = false; route = '' " type="button" class="mt-3 inline-flex w-full justify-center rounded-md bg-white dark:bg-slate-700 px-6 py-3 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</button>
          </div>
        </form>
      </x-slot>
    </x-small.modal.modal>
